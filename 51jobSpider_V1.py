...
for p in range(1,2):
    ...
    targetAddress = 'https://searchhttps://search.51job.com/list/120400,000000,0000,00,9,99,+,2,%d.html?lang=c&postchannel=0000&workyear=99&cotype=06&degreefrom=99&jobterm=99&companysize=99&ord_field=0&dibiaoid=0&line=&welfare='%(p)
    # 这里后面需要改成input手动传入
    ...
    '''
    # 输出福利
    jobWelf = re.match(r'"jobwelf":[.*?)]',str_positionList)
    A = jobWelf.group()
    print(A)
    print(len(jobWelf))
    '''
    ...
    '''
    # 输出学历要求--藏在attribute_text里面，需要再清洗一下
    #pattern_attributeText = re.compile(r'"attribute_text":(.*?)')
    #attributeText = pattern_attributeText.match(str(positionList))
    #print(attributeText)
    #print(len(attributeText))
    
    
    # 输出工作年限要求
    pattern_workYear = re.compile(r'"workyear":[.*?]')
    workYear = re.findall(pattern_workYear, string_positionList)
    print(workYear)
    print(len(workYear))
    '''
    ...
    dataframe = pd.DataFrame({'公司名称':companyName,'公司性质':companytypeText,'公司规模':companysizeText,\
                              '岗位名称':jobName,'薪资范围':providesalaryText,'工作地点':workareaText})
    dataframe.to_excel('data/Page_%d.xls'%(p),encoding='utf-8')
